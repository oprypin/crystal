...
class HTTP::WebSocket::Protocol
  ...
  def send(data : String) : IO | Int32 | Nil
    send(data.to_slice, Opcode::TEXT)
  end

  def send(data : Bytes) : IO | Int32 | Nil
  ...
  def send(data : Bytes, opcode : Opcode, flags = Flags::FINAL, flush = true) : IO | Int32 | Nil
  ...
  def close(code : CloseCode? = nil, message = nil) : Bool | IO | Int32 | Nil
    return if @io.closed?
  ...
  def close(code : Int, message = nil) : Bool | IO | Int32 | Nil
