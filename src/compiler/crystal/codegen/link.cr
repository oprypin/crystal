module Crystal
  ...
  class Program
    ...
    private def pkg_config_flags(libname, static, library_path)
      if system("pkg-config #{Process.shell_quote(libname)}")
        if static
          flags = [] of String
          `pkg-config #{Process.shell_quote(libname)} --libs --static`.split.each do |cfg|
          ...
          `pkg-config #{Process.shell_quote(libname)} --libs`.chomp
        end
      end
    end
