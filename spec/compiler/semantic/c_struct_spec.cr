...
describe "Semantic: struct" do
  it "types struct" do
    ...
    assert bar.extern? == true
    assert bar.extern_union? == false
    assert bar.instance_vars["@x"].type == mod.int32
    assert bar.instance_vars["@y"].type == mod.float64
  end
