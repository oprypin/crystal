...
describe "Code gen: def" do
  ...
  it "codegens call without args" do
    assert run("def foo; 1; end; 2; foo").to_i == 1
  end

  it "call functions defined in any order" do
    assert run("def foo; bar; end; def bar; 1; end; foo").to_i == 1
  end

  it "codegens call with args" do
    assert run("def foo(x); x; end; foo 1").to_i == 1
  end
  ...
  it "uses self" do
    assert run("struct Int; def foo; self + 1; end; end; 3.foo").to_i == 4
  end
  ...
  it "allows to change argument values" do
    assert run("def foo(x); x = 1; x; end; foo(2)").to_i == 1
  end
