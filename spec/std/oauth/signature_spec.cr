...
describe OAuth::Signature do
  describe "key" do
    it "gets when token secret is empty" do
      ...
      assert signature.key == "consumer%20secret&"
    end
    ...
    it "gets when token secret is not empty" do
      ...
      assert signature.key == "consumer%20secret&token%20secret"
    end
  end
  ...
  describe "base string" do
    it "computes without port in host" do
      ...
      assert base_string == "POST&http%3A%2F%2Fsome.host%2Fsome%2Fpath&oauth_callback%3Dsome%252Bcallback%26oauth_consumer_key%3Dconsumer_key%26oauth_nonce%3Dnonce%26oauth_signature_method%3DHMAC-SHA1%26oauth_timestamp%3D1234%26oauth_version%3D1.0"
    end
    ...
    it "computes with port in host" do
      ...
      base_string = signature.base_string request, tls, ts, "nonce"
      assert base_string == "POST&http%3A%2F%2Fsome.host%3A5678%2Fsome%2Fpath&oauth_callback%3Dsome%252Bcallback%26oauth_consumer_key%3Dconsumer_key%26oauth_nonce%3Dnonce%26oauth_signature_method%3DHMAC-SHA1%26oauth_timestamp%3D1234%26oauth_version%3D1.0"
    end
    ...
    it "computes when TLS" do
      ...
      base_string = signature.base_string request, tls, ts, "nonce"
      assert base_string == "POST&https%3A%2F%2Fsome.host%2Fsome%2Fpath&oauth_callback%3Dsome%252Bcallback%26oauth_consumer_key%3Dconsumer_key%26oauth_nonce%3Dnonce%26oauth_signature_method%3DHMAC-SHA1%26oauth_timestamp%3D1234%26oauth_version%3D1.0"
    end
  end
