...
describe HTTP::WebSocket do
  describe "receive" do
    it "can read a small text packet" do
      ...
      assert String.new(buffer[0, result.size]) == "Hello"
    end
    ...
    it "can read partial packets" do
      ...
      2.times do
        ...
        assert String.new(buffer) == "Hel"

        result = ws.receive(buffer)
        assert_text_packet result, 2, final: true
        assert String.new(buffer[0, 2]) == "lo"
      end
    end
    ...
    it "can read fragmented packets" do
      ...
      2.times do
        result = ws.receive(buffer)
        assert_text_packet result, 3, final: false
        assert String.new(buffer[0, 3]) == "Hel"
    ...
    it "read long packet" do
      ...
      assert String.new(buffer[0, 1023]) == "x" * 1023
    end
  ...
  describe "send" do
    it "sends long data with correct header" do
      ...
      ws.send(big_string)
      bytes = io.to_slice
      assert bytes.size == 10 + size # 2 bytes header, 8 bytes size, UInt16 + 1 bytes content
      assert bytes[1] == 127
      received_size = 0
      8.times { |i| received_size <<= 8; received_size += bytes[2 + i] }
      assert received_size == size
      size.times do |i|
        assert bytes[10 + i] == 'a'.ord
      end
    end
  ...
  describe "stream" do
    it "sends continuous data and splits it to frames" do
      ...
      bytes = io.to_slice
      assert bytes.size == 4 * 2 + 512 * 3 # two frames with 2 bytes header, 2 bytes size, 3 * 512 bytes content in total
      first_frame, second_frame = {bytes[0, (4 + 1024)], bytes + (4 + 1024)}
      assert (first_frame[0] & 0x80) == 0   # FINAL bit unset
      assert (first_frame[0] & 0x0f) == 0x2 # BINARY frame
      assert first_frame[1] == 126          # extended size
      received_size = 0
      2.times { |i| received_size <<= 8; received_size += first_frame[2 + i] }
      assert received_size == 1024
      received_size.times do |i|
        assert bytes[4 + i] == 'a'.ord
      end

      assert (second_frame[0] & 0x80) != 0   # FINAL bit set
      assert (second_frame[0] & 0x0f) == 0x0 # CONTINUATION frame
      assert second_frame[1] == 126          # extended size
      received_size = 0
      2.times { |i| received_size <<= 8; received_size += second_frame[2 + i] }
      assert received_size == 512
      received_size.times do |i|
        assert bytes[4 + i] == 'a'.ord
      end
    end
    ...
    it "sets opcode of first frame to binary if stream is called with binary = true" do
      ...
      bytes = io.to_slice
      assert (bytes[0] & 0x0f) == 0x2 # BINARY frame
    end
  end
  ...
  describe "send_masked" do
    it "sends the data with a bitmask" do
      ...
      ws.send(sent_string)
      bytes = io.to_slice
      assert bytes.size == 11     # 2 bytes header, 4 bytes mask, 5 bytes content
      assert bytes[1].bit(7) == 1 # For mask bit
      assert (bytes[1] - 128) == sent_string.size
      assert (bytes[2] ^ bytes[6]) == 'h'.ord
      assert (bytes[3] ^ bytes[7]) == 'e'.ord
      assert (bytes[4] ^ bytes[8]) == 'l'.ord
      assert (bytes[5] ^ bytes[9]) == 'l'.ord
      assert (bytes[2] ^ bytes[10]) == 'o'.ord
    end
    ...
    it "sends long data with correct header" do
      ...
      ws.send(big_string)
      bytes = io.to_slice
      assert bytes.size == size + 14 # 2 bytes header, 8 bytes size, 4 bytes mask, UInt16::MAX + 1 bytes content
      assert bytes[1].bit(7) == 1    # For mask bit
      assert (bytes[1] - 128) == 127
      received_size = 0
      8.times { |i| received_size <<= 8; received_size += bytes[2 + i] }
      assert received_size == size
      size.times do |i|
        assert (bytes[14 + i] ^ bytes[10 + (i % 4)]) == 'a'.ord
      end
    end
  end
  ...
  describe "close" do
    it "closes with message" do
      ...
      ws.close(message)
      bytes = io.to_slice
      assert (bytes[0] & 0x0f) == 0x8 # CLOSE frame
      assert String.new(bytes[2, bytes[1]]) == message
    end
